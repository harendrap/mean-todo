...
module.exports = function(app) {
  ...
  app.route('/api/todos/:todoId')
    ...
    // Update Todo fetched by todoById  //removed hasAuthorization
    .put(requiresLogin,  function(req, res) {
          var todo = req.todo;

          todo.title = req.body.title;
          todo.comment = req.body.comment;
          todo.completed = req.body.completed;
        todo.mappings=req.body.mappings;
        //var doc = req.body.mappings(_id);
       // todo.mappings.push(doc);
        //todo.mappings.findOneAndUpdate({hl7: req.body.mappings.$.hl7}, {$push: {'todo.mappings': req.body.mappings}});
        //todo.mappings.id(_id).set(req.body.mappings(_id));  //(req.body.mappings);
          todo.save(function(err) {
            if(err) {
              return res.status(400).send({
                message: getErrorMessage(err)
